import numpy as np
from ibc_analysis import get_icd9_cui_mappings

# using best
ingredient_to_best_ndc = {}
with open('ingredient_drug_ndc', 'r') as infile:
...
ingredient_to_ndcs = {}
with open('ingredient_ndcs', 'r') as infile:
    data = infile.readlines()
    for row in data:
        eles = row.strip().split(' ')
...
with open('ingredient_all_ndcs', 'r') as infile:
    data = infile.readlines()
    for row in data:
        eles = row.strip().split(' ')
...
ibc_filenames = []
with open('list_of_ibc_files.txt', 'r') as infile:
    data = infile.readlines()
    for row in data:
        ibc_filename = row.strip()
        ibc_filenames.append(ibc_filename)


cui_to_icd9, icd9_to_cui = get_icd9_cui_mappings()
for embedding_filename in ibc_filenames:
    ...
    '''
    outfilename = 'ibc_embeddings/' + 'best_' + embedding_filename.split('/')[-1]
    with open(outfilename, 'w') as outfile:
        for ingredient in ingredient_to_best_ndc.keys():
            best_ndc = ingredient_to_best_ndc[ingredient]
            
            outfile.write('%s %s\n' %(ingredient, ndc_to_embeddings[best_ndc]))
    '''

    outfilename = 'ibc_embeddings/' + 'more_' + embedding_filename.split('/')[-1]
    with open(outfilename, 'w') as outfile:
        for ingredient in ingredient_to_ndcs.keys():
            ndcs = ingredient_to_ndcs[ingredient]
            embeddings = []
            for ndc in ndcs:
    ...
    outfilename = 'ibc_embeddings/' + 'all_' + embedding_filename.split('/')[-1]
    with open(outfilename, 'w') as outfile:
